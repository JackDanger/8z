...
///   bits 7:6 = reserved (0)
fn write_coder_record(h: &mut Vec<u8>, coder: &crate::container::Coder) {
...
fn build_header(files: &[EncodedFile]) -> SevenZippyResult<Vec<u8>> {
    ...
    // ── PackInfo (0x06) ───────────────────────────────────────────────────────
    // num_pack_streams is the total count of all packed streams across all folders.
    // For one folder per file with one packed stream each, this equals num_files.
    h.push(0x06);
    write_uint64(&mut h, 0); // pack_pos = 0
    write_uint64(&mut h, num_files as u64); // num_pack_streams
                                            // Size (0x09)
    h.push(0x09);
    for ef in files {
